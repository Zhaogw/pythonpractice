...
output_route = "E:\\记忆\\脸谱\\选手记忆93张切割图片\\"
...
for i in range(0, len(files_list)):
    ...
    region7.save(output_route+file_name+'_7.png')

    region8 = img.crop((w, 2*h, 2*w, 3*h))
    region8.save(output_route+file_name+'_8.png')

    region9 = img.crop((2*w, 2*h, 3*w, 3*h))
    region9.save(output_route+file_name+'_9.png')
